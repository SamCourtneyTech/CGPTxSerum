...
ChatBarComponent::ChatBarComponent(CGPTxSerumAudioProcessor& p) : processor(p)
{
    ...
    sendButton.onClick = [this]() {
        ...
        DBG("First parent: " + (getParentComponent() != nullptr ? getParentComponent()->getName() : "null"));
        DBG("Second parent: " + (getParentComponent() != nullptr && getParentComponent()->getParentComponent() != nullptr
            ? getParentComponent()->getParentComponent()->getName() : "null"));

        if (auto* editor = dynamic_cast<CGPTxSerumAudioProcessorEditor*>(getParentComponent()->getParentComponent()))
        {
            DBG("Found editor, showing loading screen");
            editor->showLoadingScreen(true);
        }
        else
        {
            DBG("Could not find editor component!");
        }
        ...
        sendPromptToChatGPT(userInput, [this](std::map<std::string, std::string> response)
            {
                processor.applyPresetToSerum(response);
            });
...
{
    // Show the loading screen on the UI thread.
    if (auto* editor = dynamic_cast<CGPTxSerumAudioProcessorEditor*>(getParentComponent()->getParentComponent()))
        editor->showLoadingScreen(true);
    ...
    std::thread([this, userPrompt, callback]()
        {
            ...
            // --- Begin Synchronous API Call Logic ---
            const juce::String apiEndpoint = "https://api.openai.com/v1/chat/completions";
            const juce::String apiKey = "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";
            constexpr int MAX_OPENAI_LENGTH = 32000;
            constexpr int CONNECTION_TIMEOUT_MS = 90000;  // 90 seconds
            constexpr int READ_TIMEOUT_MS = 90000;          // 90 seconds

            // System instruction string (shortened for brevity – replace with your full text)
            ...
            // Convert JSON payload to a compact string.
            ...
            // Create the WebInputStream for the POST request.
            ...
            if (!webStream->connect(nullptr))
            {
                ...
                juce::MessageManager::callAsync([this, callback, parameterMap]()
                    {
                            ...
                            if (choiceObj->hasProperty("message"))
                            {
                                ...
                                if (auto* msgObj = msg.getDynamicObject())
                                {
                                    if (msgObj->hasProperty("content"))
                                    {
                                        ...
                                        // Parse the content to fill parameterMap.
            ...
            // --- End Synchronous API Call Logic ---

            // Switch back to the UI thread to hide the loading screen and return the result.
            juce::MessageManager::callAsync([this, parameterMap, callback]()
                {
                    if (auto* editor = dynamic_cast<CGPTxSerumAudioProcessorEditor*>(getParentComponent()->getParentComponent()))
                        editor->showLoadingScreen(false);
                    callback(parameterMap);
