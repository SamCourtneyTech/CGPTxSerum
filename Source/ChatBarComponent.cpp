                        ...
                        if (auto* msgObj = msg.getDynamicObject())
                        {
                            if (msgObj->hasProperty("content"))
                            {
                                ...
                                for (const juce::String& line : lines) {
                                    // 2. Find the position of the comma separating name and value:
                                    int commaPos = line.indexOf(",");

                                    if (commaPos != -1) {
                                        // 3. Extract the name and value (handling potential whitespace):
                                        juce::String name = line.substring(0, commaPos).trim();
                                        juce::String value = line.substring(commaPos + 1).trim();
                                        ...
                                        if (!line.trim().isEmpty()) {
                                            DBG("Warning: Invalid line format: " + line);
                                        }
                                    }
                                }
